...
action :create do
  ...
  execute "set #{new_resource.key} in #{new_resource.rc_conf_file}" do
    command "sysrc#{' -f ' + new_resource.rc_conf_file if new_resource.rc_conf_file} #{new_resource.key}=\"#{new_resource.value}\""
...
action :delete do
  ...
  execute "delete #{new_resource.key} from #{new_resource.rc_conf_file}" do
    ...
    cmd += " -f #{new_resource.rc_conf_file}" if new_resource.rc_conf_file'
    command cmd
